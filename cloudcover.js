...
if (sun_altitude >= 0) {
  ...
  let directIrradiance =
    1370 *
    ((1 - 0.14 * node_altitude) * Math.pow(0.7, Math.pow(airMass, 0.678)) +
      0.14 * node_altitude);
  ...
  let moduleIrradiance =
    directIrradiance * 0.2 * ((Math.PI - node_tilt) / Math.PI);
  ...
  if (
    ...
    moduleIrradiance +=
      ...
      (Math.cos(sun_altitude) *
        Math.sin(node_tilt) *
        Math.cos(node_orientation - sun_azimuth) +
