...
if (sun_altitude >= 0) {
        ...
        // angle of incidence
        const aoi = Math.acos(Math.cos(sun_zenith) * Math.cos(node_tilt) + Math.sin(sun_zenith) * Math.sin(node_tilt) * Math.cos(sun_azimuth - node_orientation))
        ...
        let ed = (dhi * ((1 + Math.cos(node_tilt)) / 2)) + (ghi * (((0.012 * sun_zenith - 0.04) * (1 - Math.cos(node_tilt))) / 2))
